...
def normalize_query(text: str) -> str:
    """Normalize query for smart cache key generation (lowercase, strip punctuation & extra spaces)."""
...
def _cache_key(prompt: str, system: str) -> str:
    norm_prompt = normalize_query(prompt)
    norm_system = normalize_query(system)
    return hashlib.sha256(f"{norm_system}:::${norm_prompt}".encode("utf-8")).hexdigest()

async def warmup_ollama():
    """Warm Ollama model during startup to ensure keep_alive='60m' is active in GPU memory."""
    try:
        logger.info(f"Pre-warming Ollama model '{settings.OLLAMA_MODEL}'...")
        ...
        logger.info(f"Ollama model '{settings.OLLAMA_MODEL}' warmed and loaded in VRAM (keep_alive: 60m).")
    except Exception as e:
        logger.warning(f"Ollama warmup notice (non-fatal, local model will load on first request): {e}")
