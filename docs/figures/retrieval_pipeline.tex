...
\node[box, below=1cm of n3] (n4) {Sentence Transformer};
...
\begin{scope}[on background layer]
    \node[container, fit=(n1) (n2) (n3) (n4) (n5) (n6)] (stage1) {};
    \node[header, anchor=south west, yshift=2pt] at (stage1.north west) {STAGE 1: QUERY EMBEDDING \& VECTOR SEED SEARCH};
\end{scope}

% ================= INPUT CONTEXT =================
% Pushed further up (2.2cm) so it cleanly clears the dashed boundary and title
\node[inputbox, above=2.2cm of n1] (input) {\textbf{INPUT CONTEXT}\\Repository Summary\\Commit Snapshots\\Temporal Metadata};
\draw[arrow, dashed, color=acmSecondary] (input) -- (n1);

% ================= STAGE 2 =================
\node[box, below=2cm of n6] (m6) {Top-K Candidate\\Retrieval};
\node[box, right=of m6] (m5) {Temporal\\Filtering};
\node[box, right=of m5] (m4) {Commit\\Ranking};

\node[box, below=1cm of m4] (m3) {Prompt Builder};
...
\begin{scope}[on background layer]
    \node[container, fit=(m1) (m2) (m3) (m4) (m5) (m6)] (stage2) {};
    \node[header, anchor=south west, yshift=2pt] at (stage2.north west) {STAGE 2: TEMPORAL RE-RANKING \& RESPONSE SYNTHESIS};
\end{scope}

% ================= OUTPUT METRICS =================
% Pushed further down (1.5cm) to clear the bottom boundary
\node[inputbox, below=1.5cm of m1] (output) {\textbf{OUTPUT METRICS}\\Evidence Score\\Confidence Score\\Source Citations\\Evolution Timeline};
