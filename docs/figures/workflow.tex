...
\begin{tikzpicture}[
    ...
    \node (url) [process, below=1.0cm of user] {Enter GitHub Repository URL};
    ...
    \node (question) [terminal, below=1.2cm of chromadb] {User Question};
    \node (intent) [decision, below=1.0cm of question] {Intent Classification\\(Overview / Historical / Bug Origin)};
    \node (retrieval) [process, below=0.8cm of intent] {Temporal Retrieval};
    ...
    \node (display) [terminal, fill=ieeePrimary, text=white, below=1.0cm of response] {
        Display Evidence + Confidence + Timeline
    };

    % Layer Grouping Containers with Header Labels Positioned Cleanly ABOVE Boxes
    \begin{scope}[on background layer]
        \node (ingest_layer) [container, fit=(url) (clone) (commits) (readme) (snapshots) (embeddings) (chromadb)] {};
        \node[anchor=south west, font=\sffamily\tiny\bfseries\color{ieeePrimary}, yshift=2pt] at (ingest_layer.north west) {STAGE 1: REPOSITORY INGESTION \& TEMPORAL INDEXING};

        \node (rag_layer) [container, fit=(intent) (retrieval) (topk) (prompt) (llm) (response)] {};
        \node[anchor=south west, font=\sffamily\tiny\bfseries\color{ieeePrimary}, yshift=2pt] at (rag_layer.north west) {STAGE 2: TEMPORAL RETRIEVAL \& LLM REASONING};
    \end{scope}
