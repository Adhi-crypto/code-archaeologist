...
% Top Row: Stage 1 (Left to Right)
\node[box, fill=acmPrimary!12] (n1) {User Query};
\node[box, right=of n1] (n2) {Normalize Query};
\node[box, right=of n2] (n3) {Intent Classifier};
\node[box, right=of n3] (n4) {Sentence Transformer};
\node[box, right=of n4] (n5) {Query Embedding};
\node[box, fill=acmSecondary!15, right=of n5] (n6) {ChromaDB\\Vector Search};
...
% Bottom Row: Stage 2 (Right to Left)
\node[box, below=1.8cm of n6] (m6) {Top-K Candidate\\Retrieval};
\node[box, left=of m6] (m5) {Temporal\\Filtering};
\node[box, left=of m5] (m4) {Commit\\Ranking};
\node[box, left=of m4] (m3) {Prompt Builder};
...
% Input Context (Left of Stage 1)
\node[inputbox, left=1.2cm of n1] (input) {\textbf{INPUT CONTEXT}\\Repository Summary\\Commit Snapshots\\Temporal Metadata};
\draw[arrow, dashed, color=acmSecondary] (input) -- (n1);

% Output Metrics (Left of Stage 2)
\node[inputbox, left=1.2cm of m1] (output) {\textbf{OUTPUT METRICS}\\Evidence Score\\Confidence Score\\Source Citations\\Evolution Timeline};
